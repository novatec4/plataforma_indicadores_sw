        ...
        <div className="overflow-y-auto max-h-96">
            <table className="w-full text-sm text-left">
                <thead className="sticky top-0 bg-slate-50 z-10">
                    <tr>
                        <th className="p-3 font-semibold text-slate-600">Año de Inicio</th>
                        <th className="p-3 font-semibold text-slate-600 text-right">Cantidad de Proyectos</th>
                    </tr>
                </thead>
                <tbody className="bg-white">
                    {chartData.map((d, index) => (
                        <tr key={d.periodo} className={index % 2 === 0 ? 'bg-white' : 'bg-slate-50/50 hover:bg-slate-100'}>
                            ...
                            <td className="p-3 text-slate-600 text-right font-mono">{d.promedio}</td>
                        </tr>
                    ))}
                </tbody>
            </table>
